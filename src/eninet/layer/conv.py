...
class TwoBodyEquiGraphConv(torch.nn.Module):
    ...
    def forward(
        ...
        with g.local_scope():
            ...
            g.update_all(
                fn.copy_e("e_v_upd", "e_v_upd"),
                agg_fn("e_v_upd", "n_ev")
            )
            g.update_all(
                fn.copy_e("e_s_upd", "e_s_upd"),
                agg_fn("e_s_upd", "n_es")
            )
...
class ThreeBodyEquiGraphConvSimple(torch.nn.Module):
    def __init__(
        ...
        self.ns_in = nn.Sequential(
            nn.Linear(g_feat_dim, lg_feat_dim),
            nn.SiLU()
        )
    ...
    def _update_edge_s(self, edges: dgl.udf.EdgeBatch):
        ...
        triplet_message = (
            edge_neighbors * self.triplet_proj(edges.data["e_s"])
        )
    ...
    def forward(
        ...
        with g.local_scope():
            g.ndata.update({
                "n_s": self.ns_in(node_s),
                "n_v": self.nv_in(node_v)
            })
            g.edata.update({
                "e_s": edge_s,
                "e_v": edge_v
