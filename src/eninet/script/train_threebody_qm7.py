...
from data.scaler import DummyScaler, RemoveMeanScaler, StandardScaler
from data.utils import collate_fn_g, collate_fn_lg
from graph.converter import Molecule2Graph, get_element_list
...
dataset = ASEDataset(
    ...
    linegraph_filename=os.path.join(
        file_savedir, f"qm7_linegraph_cutoff{cutoff}_{task}"
    ),
)
...
train_data, val_data, _ = split_dataset(
    dataset,
    frac_list=[train_ratio, 1 - train_ratio, 0.0],
    shuffle=True,
...
val_loader = DataLoader(
    val_data, batch_size=64, shuffle=False, collate_fn=collate_fn, num_workers=n_workers
)
...
earlystopping_callback = EarlyStopping(
    "val_loss", mode="min", patience=earlystopping_patience
)
...
if task == "ccsd":
    ...
    for struct_dft, struct_ccsd in zip(structures_test, structures_ccsd_test):
        ...
        labels_test.append(
            torch.tensor(
                np.stack([label_ccsd, label_dft], axis=0), dtype=DEFAULT_FLOATDTYPE
            )
        )
...
test_data = ASEDataset(
    ...
    graph_filename=os.path.join(
        file_savedir, f"qm7_showcase_graph_cutoff{cutoff}_{task}"
    ),
    label_filename=os.path.join(
        file_savedir, f"qm7_showcase_label_cutoff{cutoff}_{task}"
    ),
    linegraph_filename=os.path.join(
        file_savedir, f"qm7_showcase_linegraph_cutoff{cutoff}_{task}"
    ),
)
